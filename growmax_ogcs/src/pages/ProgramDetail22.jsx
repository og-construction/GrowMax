...
import styles from '../styles/ProgramDetail22.module.css';
import axios from 'axios';
import { baseurl } from '../api'


const ProgramDetail22 = () => {
  const location = useLocation();
  const program22 = location.state?.program || {}; // Safely access program data
  const [currentDate, setCurrentDate] = useState('');
  const [currentTime, setCurrentTime] = useState('');
  const [showModal, setShowModal] = useState(false);
  const [formData, setFormData] = useState({ name: '', mobile: '', email: '' });
  //const baseurl = 'https://yourapi.com'; 
  ...
  return (
    <div className={styles.pageContainer}>
      <div className={styles.card}>
        {/* Program Title */}
        <h1 className={styles.title}>{program22.name}</h1>
        ...
        <div className={styles.section}>
          ...
          <p className={styles.text}>{program22.description}</p>
        </div>
        ...
        <div className={styles.section}>
          ...
          <p className={styles.text}>{program22.text}</p>
        </div>
          ...
          {showModal && (
            <div className={styles.modalOverlay}>
              <div className={styles.modal}>
                <h2 className={styles.modalTitle}>Join {program22.name}</h2>
...
export default ProgramDetail22;
