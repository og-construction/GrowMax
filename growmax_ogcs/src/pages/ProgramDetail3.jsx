...
import styles from '../styles/ProgramDetail3.module.css';
import { baseurl } from '../api'

const ProgramDetail3 = () => {
  const location = useLocation();
  const program3 = location.state?.program || {}; // Safely access program data
  const [currentDate, setCurrentDate] = useState('');
  const [currentTime, setCurrentTime] = useState('');
  const [showModal, setShowModal] = useState(false);
  const [formData, setFormData] = useState({ name: '', mobile: '', email: '' });
  // const baseurl = 'http://localhost:8000'; // Replace with your API endpoint
  ...
  return (
    <div className={styles.pageContainer}>
      <div className={styles.card}>
        {/* Program Title */}
        <h1 className={styles.title}>{program3.name}</h1>
        ...
        <div className={styles.section}>
          ...
          <p className={styles.text}>{program3.description}</p>
        </div>
      ...
      {showModal && (
        <div className={styles.modalOverlay}>
          <div className={styles.modal}>
            <h2 className={styles.modalTitle}>Join {program3.name}</h2>
